...
function BCT:CreateCombatLogPanel()
    ...
    -- Removed filter buttons as they were causing issues
...
function BCT:CreateMinimapButton()
    if not config.showMinimapButton or minimapButton then return end
    
    minimapButton = CreateFrame("Button", "BCT_MinimapButton", Minimap)
    ...
    icon:SetTexture("Interface\\Icons\\Spell_Holy_SearingLight")
    ...
    minimapButton:SetScript("OnEnter", function(self)
        ...
        GameTooltip:SetText("Better Combat Text", 1, 1, 1)
        GameTooltip:AddLine("Left-click: Toggle combat log", 0.8, 0.8, 0.8)
...
-- Remove filter functionality since filter buttons were removed
-- Filter combat log function is no longer needed
...
SlashCmdList["BCT"] = function(msg)
    ...
    elseif cmd == "minimap" then
        ...
        if config.showMinimapButton then
            if not minimapButton then
                BCT:CreateMinimapButton()
            end
        ...
        print("|cff00ff00BCT:|r Minimap button " .. (config.showMinimapButton and "shown" or "hidden"))
    ...
    elseif cmd == "help" or cmd == "" then
        ...
        print("  |cff00ffff/bct minimap|r - Toggle minimap button")
