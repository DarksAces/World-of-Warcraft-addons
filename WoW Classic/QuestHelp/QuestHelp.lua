...
local function CreateHelpButton_Retail()
    ...
    btn:SetSize(70, 20)
    btn:SetText("Help")
    -- Colocamos consistente: arriba a la izquierda del panel de detalles (misma que Classic)
    btn:SetPoint("TOPLEFT", parent, "TOPLEFT", 60, -30)
...
local function CreateHelpButton_MoP()
    -- Soportes comunes en MoP Classic: QuestLogDetailFrame o QuestLogFrame
    if BQH.MoPButton then return end
    local parent = QuestLogDetailFrame or QuestLogDetailPanel or QuestLogFrame or QuestFrame
    if not parent then return end

    local btn = CreateFrame("Button", "BQH_MoPHelpButton", parent, "UIPanelButtonTemplate")
    btn:SetSize(80, 20)
    btn:SetText("Help")

    -- Usar la misma ubicación consistente que en Classic/Retail
    btn:SetPoint("TOPLEFT", parent, "TOPLEFT", 60, -30)
