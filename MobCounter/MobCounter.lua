...
local function UpdateGroupTable()
    ...
    -- Calculate container height dynamically (title + entries)
    local containerHeight = 30 + (#sortedMembers * 15)

    -- Create container frame sized to fit all entries with margin
    local container = CreateFrame("Frame", nil, frame)
    container:SetSize(frame:GetWidth() - 20, containerHeight)
    ...
    -- Entries for each member with name truncation and coloring
    ...
    for i, member in ipairs(sortedMembers) do
        ...
        entry:SetPoint("TOPLEFT", container, "TOPLEFT", 10, -30 - (i-1)*15)
        ...
        entry:SetText(i .. ". " .. nameColor .. displayName .. "|r: " .. member.kills)
        entry:SetJustifyH("LEFT")
    end

    -- Recalculate total kills and update total text
    RecalculateTotal()

    -- Adjust frame height if not manually resized
    if not frame.isResizing and not frame.hasUserResized then
    local newHeight = math.max(150, containerHeight + 50)
...
local function InitializeMainUI()
    ...
    f:SetMovable(true) -- Make the main frame movable
       f:SetResizable(true) -- Permitir que se redimensione
    
    -- Establecer límites de tamaño con SetResizeBounds (mínimo y máximo)
    if f.SetResizeBounds then
        f:SetResizeBounds(180, 150, 400, 600)
    else
        -- Para versiones antiguas de WoW que no soporten SetResizeBounds
        if f.SetMinResize then f:SetMinResize(180, 150) end
        if f.SetMaxResize then f:SetMaxResize(400, 600) end
    end
...
resizeButton:SetScript("OnMouseUp", function(self, button)
    if button == "LeftButton" then
        ...
        f.hasUserResized = true  -- Aquí señalas que el usuario redimensionó
        local width, height = f:GetSize()
        if height < 150 then height = 150; f:SetHeight(height) end
